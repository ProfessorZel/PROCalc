...
def run(con, data_id, n, IBC, table_name, inputs, params, count, RD):
    ...
    for row in RAWDATA:
        ...
        if params['max_mode'] == "left":
            ...
            for i in range(1, n + 1):
                ans['max_S_' + str(i)] = str((dc.Decimal(a['maxs_S_' + str(i) + "_pos"]) + dc.Decimal(
                    a['max_S_' + str(i) + "_pos"])) / dc.Decimal('2.0'))
        ...
        if params['min_mode'] == "left":
            ...
            for i in range(1, n + 1):
                ans['min_S_' + str(i)] = str((dc.Decimal(a['mins_S_' + str(i) + "_pos"])
                                              + dc.Decimal(a['min_S_' + str(i) + "_pos"])) / dc.Decimal('2.0'))
        else:
    ...
    if len(ANSWERS) > 0:
        ...
        for row in ANSWERS:
            arr = ["'" + str(row['t']) + "'", "'" + str(row['min_SumS']) + "'", "'" + str(row['max_SumS']) + "'"]
            for i in range(1, n + 1):
