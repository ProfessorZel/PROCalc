...
def run(con,data_id,n,IBC,table_name,inputs,params,count,RD):
    ...
    #MATH#
    if (secrun):
        X = np.arange(dc.Decimal(params['x_min'])+dc.Decimal('0.1')**dc.Decimal(params['step']), dc.Decimal(params['x_max']), dc.Decimal('0.1')**dc.Decimal(params['step']))
    else:
        X = np.arange(dc.Decimal(params['x_min']), dc.Decimal(params['x_max']), dc.Decimal('0.1')**dc.Decimal(params['step']))
    R=[]
    for x in X:
        row=[x]
