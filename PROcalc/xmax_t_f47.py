...
def run(con,data_id,n,IBC,table_name,inputs,params,count,RD):
    ...
    req = "SELECT `host1596090_math`.`data`.* FROM `data` INNER JOIN `tasks` ON (data.taskID=tasks.id) INNER JOIN `functions` ON (functions.id =tasks.function) WHERE (`functions`.`canonical_name` = 'f47') AND (`tasks`.`ibc`={0}) AND (`data`.`state` IN (2,3,6))".format(IBC['id'])
    print(req)
    cur.execute(req)
